# TROUBLESHOOTING.md — Production City

**Mandatory protocol for ALL issue handling by agents in this repository.**

This document is paired with `CODING.md` and `CODING-RUNBOOK.md`. All rules here are **NON-NEGOTIABLE**.
...
- D1 binding, adapter, or driver errors
...
### Step 3: Research IMMEDIATELY

**IMMEDIATELY** research the issue:
...
Use the agent tools available in this session (subagents, agent teams) to parallelise research where possible. Update the GitHub issue with all findings before proceeding.
...
### Step 5: Fix (If Not Deferred)

Task an agent or proceed to ship a **REAL PRODUCTION FIX**.
...
The fix MUST follow `CODING-RUNBOOK.md` completely:
...
1. **Verify fix** — confirm the fix works (check CI, deployed behaviour where applicable)
2. **Update issue** with final status
3. **Close issue** with comment: "Fixed in #<PR number>"
4. **Update project memory** if the issue revealed a pattern or learning
...
2. Research all simultaneously (Step 3)
...
4. Re-run Codex MCP review
5. Do NOT merge until review passes
